...
function moveParticles(){
  ...
  for (var i = 0; i < particles.length; i++) {
    ...
    var f = findMapping(mic.getSprectrum(100))*particle.dir*-2;
    
    particle.size = clamp(f,120,400);
    particle.size = f;
    //particle.centerx = Math.sin(frames/1000)*width;
    ...
    if(particle.kind =="line"){
      ...
      ctx.line(particle.centerx+particle.x, particle.centery+particle.y,particle.centerx+f,particle.centery);
