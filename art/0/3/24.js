...
light = new THREE.DirectionalLight( 0x0000ff );
			light.position.set( 0.5, 0.5, 50 );
			scene.add( light );

			pointLight = new THREE.PointLight( 0xffffff, 0.5, 80 );
			pointLight.position.set( 0, 0, 100 );
			scene.add( pointLight );

			ambientLight = new THREE.AmbientLight( 0x202080 );
			...
			renderer.setClearColor( 0xaa3300 );
		...
		function generateMaterials() {
			...
			"shiny"  :
			{
				m: new THREE.MeshPhongMaterial( { color: 0xffffff, specular: 0x220000, 
					//envMap: reflectionCube, 
					combine: THREE.MixOperation, reflectivity: 0.1, metal: true } ),
				h: 0, s: 0.8, l: 0.2
			},

			"matte" :
			{
				m: new THREE.MeshPhongMaterial( { color: 0x0000ff, specular: 0x111111, shininess: 1 } ),
				h: 0, s: 0, l: 1
			},
			...
			"plastic" :
			{
				m: new THREE.MeshPhongMaterial( { color: 0x0000ff, specular: 0x888888, ambient: 0x0000ff, shininess: 50 } ),
				h: 0.1, s: 0.1, l: 0.5
			},
		...
		function updateCubes( object, time, numblobs, floor, wallx, wallz ) {
			...
			for ( i = 0; i < numblobs; i ++ ) {
				...
				ballx = Math.sin( i + 1.26 * time * ( 1.03 + 0.1 * Math.cos( 0.21 * i ) ) ) * 0.27 + 0.5;
				bally = Math.abs( Math.cos( i + 1.12 * time * Math.cos( 1.22 + 0.1424 * i ) ) ) * 0.77; // dip into the floor
				ballz = Math.cos( i/10 + 1.32 * time * 0.1 * Math.sin( ( 0.92 + 0.53 * i ) ) ) * 0.27 + 0.5;

				object.addBall(ballx, bally, ballz, strength, subtract);

			}

			if ( floor ) object.addPlaneY( 5, 32 );
			if ( wallz ) object.addPlaneZ( 5, 32 );
			if ( wallx ) object.addPlaneX( 5, 32 ); 
