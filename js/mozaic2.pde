...
SampleColour source_colors,dest_colors;
String source = "images/bball2.jpg";
String dest = "images/bball.jpg";

void setup() {
  size(window.innerWidth,window.innerHeight);
  source_img = loadImage(source);
  dest_img = loadImage(dest);
  source_colors = new SampleColour(numberColours, source);
  dest_colors = new SampleColour(numberColours, dest);
  // setUpSourceImage();
  // setUpDestImage();
  // background(255);
  // noStroke();
  ...
  void sampleColour() {
    ...
    for (int x=0; x < img.width; x+=20){
      for (int y=0; y < img.height; y+=20) {
       color c = get(x,y);
       if (checkDuplicates(c, count)) continue;
