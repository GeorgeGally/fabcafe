...
int pixelSize = 20;
String mainImage = "images/ozzie.jpg";
...
void setup(){
  ...
  colorMode(RGB, 255);
  //background(bgPic);
  source_img = loadImage("images/cats/2.png");
  getBlocks();
...
function getBlocks(){
  ...
  for (var y = 0; y < source_img.height; y+=pixelSize) {
     for (var x = 0; x < source_img.width; x+=pixelSize) {
      pics[i] = get(x, y, pixelSize, pixelSize);
